...
function printHelp () {
  ...
  echo "    -c <channel name> - test network channel name (defaults to \"ehrchannel\")"
  ...
  echo "	addOrg3.sh up"
  echo "	addOrg3.sh up -c ehrchannel -s couchdb"
...
function generateOrg3Definition() {
  ...
  export FABRIC_CFG_PATH=$PWD
  set -x
  configtxgen -printOrg Org3MSP > ../organizations/peerOrganizations/org3.ehr.com/org3.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org3.ehr.com" ]; then
    generateOrg3
