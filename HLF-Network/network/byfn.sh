...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"ehrchannel\")"
  ...
  echo "    -i <imagetag> - the tag to be used to launch the network (defaults to \"ehrchannel\")"
  ...
  echo
  echo "	byfn.sh generate -c ehrchannel"
  echo "	byfn.sh up -c ehrchannel -s couchdb"
  echo "        byfn.sh up -c ehrchannel -s couchdb -i 1.4.0"
  echo "	byfn.sh up -l javascript"
  echo "	byfn.sh down -c ehrchannel"
  echo "        byfn.sh upgrade -c ehrchannel"
  echo
...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/hospitals.ehr.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/patients.ehr.com/ca && ls *_sk)
  fi
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.ehr.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.ehr.com
    docker cp -a orderer.ehr.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.ehr.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.ehr.com

    for PEER in peer0.hospitals.ehr.com peer1.hospitals.ehr.com peer0.patients.ehr.com peer1.patients.ehr.com; do
  ...
  ./connection-profile/ccp-generate.sh
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/network --rm hyperledger/fabric-tools:$IMAGETAG rm -Rf /tmp/first-network/ledgers-backup
...
# (``peer0.org1.ehr.com`` & ``peer0.org2.ehr.com``).  Secondly, we point to
...
COMPOSE_FILE=docker/docker-compose-cli.yaml
#
...
if [ "${MODE}" == "up" ]; then
  networkUp
elif [ "${MODE}" == "down" ]; then ## Cxlear the network
  networkDown
