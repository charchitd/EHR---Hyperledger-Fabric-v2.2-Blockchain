...
// peer chaincode invoke -C ehrchannel -n marblesp -c '{"Args":["InitMarble"]}' --transient "{\"marble\":\"$MARBLE\"}"
//
...
// peer chaincode invoke -C ehrchannel -n marblesp -c '{"Args":["TransferMarble"]}' --transient "{\"marble_owner\":\"$MARBLE_OWNER\"}"
//
// export MARBLE_DELETE=$(echo -n "{\"name\":\"marble1\"}" | base64 | tr -d \\n)
// peer chaincode invoke -C ehrchannel -n marblesp -c '{"Args":["Delete"]}' --transient "{\"marble_delete\":\"$MARBLE_DELETE\"}"

// ==== Query marbles, since queries are not recorded on chain we don't need to hide private data in transient map ====
// peer chaincode query -C ehrchannel -n marblesp -c '{"Args":["ReadMarble","marble1"]}'
// peer chaincode query -C ehrchannel -n marblesp -c '{"Args":["ReadMarblePrivateDetails","marble1"]}'
// peer chaincode query -C ehrchannel -n marblesp -c '{"Args":["GetMarblesByRange","marble1","marble4"]}'

// Query a marble's public data hash
//	peer chaincode query -C ehrchannel -n marblesp -c '{"Args":["GetMarbleHash","collectionMarbles","marble1"]}'

// Rich Query (Only supported if CouchDB is used as state database):
//   peer chaincode query -C ehrchannel -n marblesp -c '{"Args":["QueryMarblesByOwner","tom"]}'
//   peer chaincode query -C ehrchannel -n marblesp -c '{"Args":["QueryMarbles","{\"selector\":{\"owner\":\"tom\"}}"]}'
...
//   peer chaincode query -C ehrchannel -n marblesp -c '{"Args":["QueryMarbles","{\"selector\":{\"docType\":\"marble\",\"owner\":\"tom\"}, \"use_index\":[\"_design/indexOwnerDoc\", \"indexOwner\"]}"]}'

// Rich Query with index design doc specified only (Only supported if CouchDB is used as state database):
//   peer chaincode query -C ehrchannel -n marblesp -c '{"Args":["QueryMarbles","{\"selector\":{\"docType\":{\"$eq\":\"marble\"},\"owner\":{\"$eq\":\"tom\"},\"size\":{\"$gt\":0}},\"fields\":[\"docType\",\"owner\",\"size\"],\"sort\":[{\"size\":\"desc\"}],\"use_index\":\"_design/indexSizeSortDoc\"}"]}'
