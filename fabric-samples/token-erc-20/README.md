...
You can use the test network script to deploy the ERC-20 token contract to the channel that was just created. Deploy the smart contract to `ehrchannel` using the following command:
...
```
peer chaincode invoke $TARGET_TLS_OPTIONS -C ehrchannel -n token_erc20 -c '{"function":"Mint","Args":["5000"]}'
```
...
```
peer chaincode query -C ehrchannel -n token_erc20 -c '{"function":"ClientAccountBalance","Args":[]}'
```
...
```
peer chaincode query -C ehrchannel -n token_erc20 -c '{"function":"ClientAccountID","Args":[]}'
```
...
peer chaincode invoke $TARGET_TLS_OPTIONS -C ehrchannel -n token_erc20 -c '{"function":"Transfer","Args":[ "'"$RECIPIENT"'","100"]}'
```
...
peer chaincode invoke $TARGET_TLS_OPTIONS -C ehrchannel -n token_erc20 -c '{"function":"Approve","Args":[ "'"$SPENDER"'","500"]}'
```
...
peer chaincode query -C ehrchannel -n token_erc20 -c '{"function":"Allowance","Args":["'"$MINTER"'", "'"$SPENDER"'"]}'
```
...
export MINTER="eDUwOTo6Q049bWludGVyLE9VPWNsaWVudCxPPUh5cGVybGVkZ2VyLFNUPU5vcnRoIENhcm9saW5hLEM9VVM6OkNOPWNhLm9yZzEuZXhhbXBsZS5jb20sTz1vcmcxLmV4YW1wbGUuY29tLEw9RHVyaGFtLFNUPU5vcnRoIENhcm9saW5hLEM9VVM="
export RECIPIENT="eDUwOTo6Q049cmVjaXBpZW50LE9VPWNsaWVudCxPPUh5cGVybGVkZ2VyLFNUPU5vcnRoIENhcm9saW5hLEM9VVM6OkNOPWNhLm9yZzIuZXhhbXBsZS5jb20sTz1vcmcyLmV4YW1wbGUuY29tLEw9SHVyc2xleSxTVD1IYW1wc2hpcmUsQz1VSw=="
peer chaincode invoke $TARGET_TLS_OPTIONS -C ehrchannel -n token_erc20 -c '{"function":"TransferFrom","Args":[ "'"$MINTER"'", "'"$RECIPIENT"'", "100"]}'
```
...
```
peer chaincode query -C ehrchannel -n token_erc20 -c '{"function":"BalanceOf","Args":["'"$MINTER"'"]}'
```
