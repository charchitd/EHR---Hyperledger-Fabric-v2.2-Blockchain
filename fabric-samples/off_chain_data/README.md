...
a CA for each org. The command also creates a channel named `ehrchannel`. The
...
the configuration blocks of `ehrchannel` in addition to the blocks that recorded
...
channel `ehrchannel`. The listener writes each block added to the channel to a
...
ehrchannel did not any data to the database because the blocks did not contain a
read-write set.
...
a file named `ehrchannel_basic.log`. This allows you to record a history of
...
after the channelid and chaincodeid, and is named ehrchannel_basic in this
example. You can navigate to this table using your browser:
http://127.0.0.1:5990/ehrchannel_basic/_all_docs
...
history to the name of the first table, and is named ehrchannel_basic_history
in this example. You can also navigate to this table using your browser:
http://127.0.0.1:5990/ehrchannel_basic_history/_all_docs
...
```
curl -X PUT http://127.0.0.1:5990/ehrchannel_basic/_design/colorviewdesign -d '{"views":{"colorview":{"map":"function (doc) { emit(doc.color, 1);}","reduce":"function ( keys , values , combine ) {return sum( values )}"}}}' -H 'Content-Type:application/json'
```

Execute a query to retrieve the total number of assets (reduce function):

```
curl -X GET http://127.0.0.1:5990/ehrchannel_basic/_design/colorviewdesign/_view/colorview?reduce=true
```
...
```
curl -X GET http://127.0.0.1:5990/ehrchannel_basic/_design/colorviewdesign/_view/colorview?group=true
```
...
```
curl -X POST http://127.0.0.1:5990/ehrchannel_basic_history/_index -d '{"index":{"fields":["blocknumber", "sequence", "key"]},"name":"asset_history"}'  -H 'Content-Type:application/json'
```
...
```
curl -X POST http://127.0.0.1:5990/ehrchannel_basic_history/_find -d '{"selector":{"key":{"$eq":"asset110"}}, "fields":["blocknumber","is_delete","value"],"sort":[{"blocknumber":"asc"}, {"sequence":"asc"}]}'  -H 'Content-Type:application/json'
```
