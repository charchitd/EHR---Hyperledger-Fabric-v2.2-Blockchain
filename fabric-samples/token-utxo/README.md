...
The test network is deployed with two peer organizations. The `createChannel` flag deploys the network with a single channel named `ehrchannel` with Org1 and Org2 as channel members.
...
You can use the test network script to deploy the UTXO token contract to the channel that was just created. Deploy the smart contract to `ehrchannel` using the following command:
```
...
```
peer chaincode invoke $TARGET_TLS_OPTIONS -C ehrchannel -n token_utxo -c '{"function":"Mint","Args":["5000"]}'
```
...
```
peer chaincode query -C ehrchannel -n token_utxo -c '{"function":"ClientUTXOs","Args":[]}'
```
...
```
peer chaincode query -C ehrchannel -n token_utxo -c '{"function":"ClientID","Args":[]}'
```
